 ...
 *
 * With Doppler (server project, prd config — exposes ATLAS_INTERNAL_API_KEY):
 *
...
function resolveAtlasApiKey(): string | undefined {
  return (
    ...
    process.env.SLASH_ATLAS_API_KEY
  );
}
...
async function main() {
  ...
  try {
    ...
    for (const a of agents.slice(0, 20)) {
      console.log(`  - ${a.path}${a.description ? `: ${a.description.slice(0, 80)}` : ""}`);
    }
    if (agents.length > 20) {
  ...
  try {
    ...
    console.log(
      `inspect(${agentPath}) → ${inspectToolNames.length} tool(s)`,
    );
    if (info?.description) {
