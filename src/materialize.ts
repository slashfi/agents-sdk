...
export function generateRootTypes(
  ...
  const lines: string[] = [
    ...
    `//`,
    `// Include in tsconfig or add:`,
    `//   /// <reference path="${configDir}/adk.d.ts" />`,
    `//`,
  ...
  const typedRefs: Array<{ name: string; interfaceName: string }> = [];
  ...
  for (const ref of syncResult.refs) {
    ...
    try {
      JSON.parse(require("node:fs").readFileSync(agentJsonPath, "utf-8"));
      typedRefs.push({
        name: ref.name,
        interfaceName: pascalCase(ref.name) + "Tools",
      });
    } catch {
      continue;
    }
